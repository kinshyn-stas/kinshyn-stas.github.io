...
class ControllerExtensionFeedYandexMarket extends Controller {
		...
		if ($this->config->get('yandex_market_status')) {
			...
			foreach ($currencies as $currency) {
				if ($currency['code'] != $offers_currency && $currency['status'] == 1) {
					$this->setCurrency($currency['code'], number_format($this->currency->convert($currency['value'], $offers_currency, $shop_currency), 4, '.', ''));
				}
			}
	...
	private function setCurrency($id, $rate = 'CBRF', $plus = 0) {
		...
		if (!in_array($id, $this->supported_currencies)) {
			return false;
		}
	 ...
	 * Товары
	 *
	 * @param array $data - параметры товара
	 */
