...
class ControllerExtensionFeedYandexMarket extends Controller {
	...
	private $error = array();
	private $supported_currencies = array('RUR', 'RUB', 'BYN', 'KZT', 'UAH', 'USD', 'EUR');
	...
	public function index() {
		...
		if (isset($this->request->post['yandex_market_in_stock'])) {
			...
			$data['yandex_market_in_stock'] = 7; // Default In Stock id
		}
		...
		if (isset($this->request->post['yandex_market_out_of_stock'])) {
			...
			$data['yandex_market_out_of_stock'] = 5; // Default Out of Sock id
		}
		...
		$allowed_currencies = array_flip($this->supported_currencies);
