<?php
class ControllerExtensionModuleInformationExtend extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('extension/module/information_extend');
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/module/information_extend', 'token=' . $this->session->data['token'], 'SSL')
		);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/module/information_extend', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);
		}

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/information_extend', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('extension/module/information_extend', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
		...
		$this->response->setOutput($this->load->view('extension/module/information_extend.tpl', $data));
	}
    ...
    //   $this->model_user_user_group->addPermission($this->user->getGroupId(), 'access', 'extension/module/information_extend');
    //   $this->model_user_user_group->addPermission($this->user->getGroupId(), 'modify', 'extension/module/information_extend');
    ...
    //   $this->model_user_user_group->addPermission($this->user->getGroupId(), 'modify', 'catalog/information_extend');
        ...
        $this->response->redirect( $this->url->link('extension/module/information_extend', 'token=' . $this->session->data['token'], 'SSL'));
    }
	...
	protected function validate() {
		...
		if (!$this->user->hasPermission('modify', 'extension/module/information_extend')) {
