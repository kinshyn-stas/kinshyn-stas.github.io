...
class ModelToolExportImport extends Model {
	...
	protected function storeCategoryIntoDatabase( &$category, &$languages, $exist_meta_title, &$layout_ids, &$available_store_ids, &$url_alias_ids ) {
		...
		$meta_h1s = $category['meta_h1s']; //added
		...
		foreach ($languages as $language) {
			...
			$meta_h1 = isset($meta_h1s[$language_code]) ? $this->db->escape($meta_h1s[$language_code]) : ''; //added
			$meta_keyword = isset($meta_keywords[$language_code]) ? $this->db->escape($meta_keywords[$language_code]) : '';
			if ($exist_meta_title) {
				$sql  = "INSERT INTO `".DB_PREFIX."category_description` (`category_id`, `language_id`, `name`, `description`, `meta_title`, `meta_description`, `meta_h1`, `meta_keyword`) VALUES ";
				$sql .= "( $category_id, $language_id, '$name', '$description', '$meta_title', '$meta_description', '$meta_h1', '$meta_keyword' );";
			} else {
				$sql  = "INSERT INTO `".DB_PREFIX."category_description` (`category_id`, `language_id`, `name`, `description`, `meta_description`, `meta_h1`, `meta_keyword`) VALUES ";
				$sql .= "( $category_id, $language_id, '$name', '$description', '$meta_description', '$meta_h1', '$meta_keyword' );";
			}
			$this->db->query( $sql );
		}
		if (!$this->use_table_seo_url) {
	...
	protected function uploadCategories( &$reader, $incremental, &$available_category_ids=array() ) {
		...
		for ($i=0; $i<$k; $i+=1) {
            ...
            //added
            $meta_h1s = array();
			while ($this->startsWith($first_row[$j-1],"meta_h1(")) {
				$language_code = substr($first_row[$j-1],strlen("meta_h1("),strlen($first_row[$j-1])-strlen("meta_h1(")-1);
				$meta_h1 = $this->getCell($data,$i,$j++);
				$meta_h1 = htmlspecialchars( $meta_h1 );
				$meta_h1s[$language_code] = $meta_h1;
			}
            //added
			$meta_keywords = array();
			...
			$category['meta_h1s'] = $meta_h1s; //added
	...
	protected function storeProductIntoDatabase( &$product, &$languages, &$product_fields, $exist_table_product_tag, $exist_meta_title, &$layout_ids, &$available_store_ids, &$manufacturers, &$weight_class_ids, &$length_class_ids, &$url_alias_ids ) {
		...
		$meta_h1s = $product['meta_h1s']; //added
		...
		foreach ($languages as $language) {
			...
			if ($exist_table_product_tag) {
				if ($exist_meta_title) {
					$sql  = "INSERT INTO `".DB_PREFIX."product_description` (`product_id`, `language_id`, `name`, `description`, `meta_title`, `meta_description`, `meta_h1`, `meta_keyword`) VALUES ";
					$sql .= "( $product_id, $language_id, '$name', '$description', '$meta_title', '$meta_description', '$meta_h1', '$meta_keyword' );";
				} else {
					$sql  = "INSERT INTO `".DB_PREFIX."product_description` (`product_id`, `language_id`, `name`, `description`, `meta_description`, `meta_h1`, `meta_keyword`) VALUES ";
					$sql .= "( $product_id, $language_id, '$name', '$description', '$meta_description', '$meta_h1', '$meta_keyword' );";
				}
				$this->db->query( $sql );
				...
				if ($exist_meta_title) {
					...
					$sql  = "INSERT INTO `".DB_PREFIX."product_description` (`product_id`, `language_id`, `name`, `description`, `meta_description`, `meta_h1`, `meta_keyword`, `tag`) VALUES ";
					$sql .= "( $product_id, $language_id, '$name', '$description', '$meta_description', '$meta_h1', '$meta_keyword', '$tag' );";
				}
				$this->db->query( $sql );
			}
		}
        ...
        if ($main_category) { //added
            $this->db->query("UPDATE " . DB_PREFIX . "product_to_category SET main_category=1 WHERE category_id = '" . $main_category . "' AND product_id = '" . $product_id . "'");
        } //added
		if (!$this->use_table_seo_url) {
			if ($keyword) {
	...
	protected function uploadProducts( &$reader, $incremental, &$available_product_ids=array() ) {
        ...
        if ($this->config->get('config_stock_status_id')) { //added
            $default_stock_status_id = $this->config->get('config_stock_status_id');
        } else {
            $default_stock_status_id = 5; //хардкод, так как нет config_stock_status_id в осисторе 2.1.0.1
        } //added
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$main_category = $this->getCell($data,$i,$j++,''); //added
			...
			$product['main_category'] = $main_category; //added
			...
			$product['meta_h1s'] = $meta_h1s; //added
	...
	protected function validateCategories( &$reader ) {
		...
		if ($exist_meta_title) {
				...
				( "category_id", "parent_id", "name", "top", "columns", "sort_order", "image_name", "date_added", "date_modified", "description", "meta_title", "meta_description", "meta_h1", "meta_keywords", "store_ids", "layout", "status" );
			} else {
				$expected_heading = array
				( "category_id", "parent_id", "name", "top", "columns", "sort_order", "image_name", "date_added", "date_modified", "description", "meta_description", "meta_h1", "meta_keywords", "store_ids", "layout", "status" );
			}
		} else {
			if ($exist_meta_title) {
				$expected_heading = array
				( "category_id", "parent_id", "name", "top", "columns", "sort_order", "image_name", "date_added", "date_modified", "seo_keyword", "description", "meta_title", "meta_description", "meta_h1", "meta_keywords", "store_ids", "layout", "status" );
			} else {
				$expected_heading = array
				( "category_id", "parent_id", "name", "top", "columns", "sort_order", "image_name", "date_added", "date_modified", "seo_keyword", "description", "meta_description", "meta_h1", "meta_keywords", "store_ids", "layout", "status" );
			}
		}
		if ($exist_meta_title) {
			$expected_multilingual = array( "name", "description", "meta_title", "meta_description", "meta_h1", "meta_keywords" );
		} else {
			$expected_multilingual = array( "name", "description", "meta_description", "meta_h1", "meta_keywords" );
		}
	...
	protected function validateProducts( &$reader ) {
		...
		$expected_heading = array
		( "product_id", "name", "categories", "main_category", "sku", "upc" ); //added
		if (in_array("ean",$product_fields)) {
		...
		$expected_heading = array_merge( $expected_heading, array( "meta_description", "meta_h1", "meta_keywords", "stock_status_id", "store_ids", "layout", "related_ids", "tags", "sort_order", "subtract", "minimum" ) );
		if ($exist_meta_title) {
			$expected_multilingual = array( "name", "description", "meta_title", "meta_description", "meta_h1", "meta_keywords", "tags" );
		} else {
			$expected_multilingual = array( "name", "description", "meta_description", "meta_h1", "meta_keywords", "tags" );
		}
		return $this->validateHeading( $data, $expected_heading, $expected_multilingual );
	}
...
//		}
		...
		foreach ($languages as $language) {
			...
			foreach ($results->rows as $key=>$row) {
				if (isset($category_descriptions[$language_code][$key])) {
					...
					$results->rows[$key]['meta_h1'][$language_code] = $category_descriptions[$language_code][$key]['meta_h1']; //added
					...
					$results->rows[$key]['meta_h1'][$language_code] = '';//added
	...
	protected function populateCategoriesWorksheet( &$worksheet, &$languages, &$box_format, &$text_format, $offset=null, $rows=null, &$min_id=null, &$max_id=null ) {
        ...
        foreach ($languages as $language) { //added
			$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('meta_h1'),32)+1);
		} //added
		foreach ($languages as $language) {
		...
		foreach ($categories as $row) {
            ...
            foreach ($languages as $language) {  //added
				$data[$j++] = html_entity_decode($row['meta_h1'][$language['code']],ENT_QUOTES,'UTF-8');
			} //added
			foreach ($languages as $language) {
		...
		foreach ($languages as $language) {
			...
			foreach ($results->rows as $key=>$row) {
				if (isset($product_descriptions[$language_code][$key])) {
					...
					$results->rows[$key]['meta_h1'][$language_code] = $product_descriptions[$language_code][$key]['meta_h1']; //added
	...
	protected function populateProductsWorksheet( &$worksheet, &$languages, $default_language_id, &$price_format, &$box_format, &$weight_format, &$text_format, $offset=null, $rows=null, &$min_id=null, &$max_id=null) {
        ...
        foreach ($languages as $language) { //added
            $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('meta_h1')+4,32)+1);
        } //added
		foreach ($languages as $language) {
		...
		foreach ($products as $row) {
            $main_category_query = $this->db->query("SELECT category_id FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . (int)$row['product_id'] . "' AND main_category = '1' LIMIT 1"); //added
            $main_category = $main_category_query->num_rows ? (int)$main_category_query->row['category_id'] : ''; //added
			$data = array();
			$worksheet->getRowDimension($i)->setRowHeight(26);
			$product_id = $row['product_id'];
			$data[$j++] = $product_id;
