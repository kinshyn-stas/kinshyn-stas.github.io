	...
	$.ajax({
		...
		success: function(json) {
			...
			if (json['error']) {
				...
				// Payment Methods
				$.ajax({
					url: '<?php echo $catalog; ?>index.php?route=api/payment/methods&order_id=<?php echo $order_id; ?>&customer_group_id=' + $('select[name=\'customer_group_id\']').val()+'&token=' + token + '&store_id=' + $('select[name=\'store_id\'] option:selected').val(),
					dataType: 'json',
					crossDomain: true,
					beforeSend: function() {
						$('#button-payment-address').button('loading');
