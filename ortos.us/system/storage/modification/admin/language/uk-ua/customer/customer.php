<?php
//Переклад: Том’як Олег tomjakoleg@ukr.net з любов’ю до Української мови та легкості Opencart
// Heading
$_['heading_title']         = 'Покупці';

// Text
$_['text_success']          = 'Інформація про Покупця успішно змінена!';
$_['text_list']             = 'Перелік Покупців';
$_['text_add']              = 'Додавання Покупця';
$_['text_edit']             = 'Редагування Покупця';
$_['text_default']          = 'За замовчуванням';
$_['text_balance']          = 'Баланс';
$_['text_add_ban_ip']       = 'Заблокувати IP';
$_['text_remove_ban_ip']    = 'Разблокувати IP';

// Column
$_['column_name']           = 'Ім’я Покупця';
$_['column_email']          = 'E-Mail';
$_['column_customer_group'] = 'Група Покупця';
$_['column_status']         = 'Статус';
$_['column_date_added']     = 'Дата додавання';
$_['column_comment']        = 'Коментар';
$_['column_description']    = 'Описи';
$_['column_amount']         = 'Сума';
$_['column_points']         = 'Бонусні бали';
$_['column_ip']             = 'IP';
$_['column_total']          = 'Аккаунтів на цьому IP';
$_['column_action']         = 'Дія';

// Entry
$_['entry_customer_group']  = 'Група Покупця';
$_['entry_firstname']       = 'Ім’я';
$_['entry_lastname']        = 'Прізвище';
$_['entry_email']           = 'E-Mail';
$_['entry_telephone']       = 'Телефон';
$_['entry_fax']             = 'Факс';
$_['entry_newsletter']      = 'Підписка на розсилку';
$_['entry_status']          = 'Статус';
$_['entry_approved']        = 'Затверджен';
$_['entry_safe']            = 'Безпечний';
$_['entry_password']        = 'Пароль';
$_['entry_confirm']         = 'Підтвердження';
$_['entry_company']         = 'Компанія';
$_['entry_address_1']       = 'Адреса 1';
$_['entry_address_2']       = 'Адреса 2';
$_['entry_city']            = 'Отделение Новой почты';
$_['entry_postcode']        = 'Індекс';
$_['entry_country']         = 'Область';
$_['entry_zone']            = 'Город';
$_['entry_default']         = 'Адреса за замовчуванням';
$_['entry_comment']         = 'Коментар';
$_['entry_description']     = 'Описи';
$_['entry_amount']          = 'Сума';
$_['entry_points']          = 'Бонусні бали';
$_['entry_name']            = 'Ім’я Покупця';
$_['entry_ip']              = 'IP';
$_['entry_date_added']      = 'Дата додавання';

// Help
$_['help_safe']             = 'Увімкніть, щоби система по боротьбі з шахрайством - ігнорувала цього Покупця.';
$_['help_points']           = 'Використовуйте мінус - щоби видалити бонусні балі';

// Error
$_['error_warning']         = 'Увага : Перевірте форму на наявність помилок!';
$_['error_permission']      = 'Увага: Ви не маєте доступу до зміни інформації про Покупця!';
$_['error_exists']          = 'Увага : Адреса електронної пошти вже зареєстрована!';
$_['error_firstname']       = 'Ім’я має бути від 1 до 32 символів!';
$_['error_lastname']        = 'Прізвище має бути від 1 до 32 символів!';
$_['error_email']           = 'Невірна адреса електронної пошти!';
$_['error_telephone']       = 'Телефон має бути від 3 до 32 символів!';
$_['error_password']        = 'Пароль має бути від 4 до 20 символів!';
$_['error_confirm']         = 'Пароль і підтвердження пароля не збігаються!';
$_['error_address_1']       = 'Адреса має містити від 3 до 128 знаків!';
$_['error_city']            = 'Отделение должно быть 2 до 128 символов!';
$_['error_postcode']        = 'Поштовий індекс має містити від 2 до 10 символів!';
$_['error_country']         = 'Укажите область!';
$_['error_zone']            = 'Укажите город!';
$_['error_custom_field']    = '%s необхідний!';
$_['error_comment']         = 'Необхідно вказати коментар!';
