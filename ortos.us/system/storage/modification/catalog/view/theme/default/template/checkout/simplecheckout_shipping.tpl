    ...
    <div class="simplecheckout-block-content">
        <?php if (!empty($shipping_methods)) { ?>
            <?php if ($display_type == 2 ) { ?>
                    ...
                    <?php foreach ($shipping_methods as $shipping_method) { ?>
                        ...
                        <?php if (empty($shipping_method['error'])) { ?>
                            <?php foreach ($shipping_method['quote'] as $quote) { ?>
                                <option value="<?php echo $quote['code']; ?>" <?php echo !empty($quote['dummy']) ? 'disabled="disabled"' : '' ?> <?php echo !empty($quote['dummy']) ? 'data-dummy="true"' : '' ?> <?php if ($quote['code'] == $code) { ?>selected="selected"<?php } ?>>
                <?php 
					if (isset($quote['image']) and !empty($quote['image']) ) {
                ?>
                    <img src="<?php echo $quote['image'];?>" class="1"/>
                <?php
				...
				?>
            <?php echo !empty($quote['text']) && !$hide_cost ? ' - '.$quote['text'] : ''; ?></option>
            ...
            <?php } else { ?>
             ...
             <?php echo !empty($quote['dummy']) ? 'disabled="disabled"' : '' ?> <?php echo !empty($quote['dummy']) ? 'data-dummy="true"' : '' ?> value="<?php echo $quote['code']; ?>" id="<?php echo $quote['code']; ?>" <?php if ($quote['code'] == $code) { ?>checked="checked"<?php } ?> />
				...
				?>
            <?php echo !empty($quote['text']) && !$hide_cost ? ' - ' . $quote['text'] : ''; ?>
                                </label>
                            </div>
