...
class ModelToolSimpleGeo extends Model {
    ...
    public function getGeoList($city) {
        ...
        if (!$geo_data) {
            ...
            $simple_geo_with_levels = $this->cache->get('simple_geo_with_levels');
            
            if (!$simple_geo_with_levels) {
                $query = $this->db->query("SHOW TABLES LIKE 'simple_geo'");

                if ($query->num_rows) {
                    $query = $this->db->query('SHOW COLUMNS FROM simple_geo LIKE \'level\'');

                    if ($query->num_rows) {
                        $this->cache->set('simple_geo_with_levels', 'yes');
                    } else {
                        $this->cache->set('simple_geo_with_levels', 'no');
                    }
                }
            }
            ...
            if ($this->cache->get('simple_geo_with_levels') == 'yes') {
                $use_level = true;
            }

            $sql = "SELECT g.id,g.fullname,g.name,g.postcode,g.zone_id FROM simple_geo g WHERE g.name LIKE '" . $this->db->escape($city) . "%' ORDER BY " . ($use_level ? "level, " : "") . "name LIMIT 25";
            /*$sql = "SELECT
                f1.fias_id AS id,
