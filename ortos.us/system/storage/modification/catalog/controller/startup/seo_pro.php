...
/*Opencart Gallery*/
        ...
        } elseif ($url[0] == 'smbpp_category_id') {
        ...
        // start: OCdevWizard SMBPP
          ...
          if (!isset($this->request->get['smbpp_path'])) {
            $path = $this->getSMBPPPathByPost($this->request->get['smbpp_post_id']);
            if ($path) $this->request->get['smbpp_path'] = $path;
          }
	...
	public function rewrite($link) {
        ...
        // start: OCdevWizard SMBPP
        case 'extension/ocdevwizard/smart_blog_pro_plus/post':
          if (isset($data['smbpp_post_id'])) {
            $tmp = $data;
            $data = array();
            if ($this->config->get('config_seo_url_include_path')) {
              $data['smbpp_path'] = $this->getSMBPPPathByPost($tmp['smbpp_post_id']);
              if (!$data['smbpp_path']) return $link;
            }
            $data['smbpp_post_id'] = $tmp['smbpp_post_id'];
            $seo_pro_utm = preg_replace('~
?
~', "
", $this->config->get('config_seo_pro_utm'));
            $allowed_parameters = explode("
", $seo_pro_utm);
        ...
        case 'extension/ocdevwizard/smart_blog_pro_plus/category':
          if (isset($data['smbpp_path'])) {
            $category = explode('_', $data['smbpp_path']);
            $category = end($category);
            $data['smbpp_path'] = $this->getSMBPPPathByCategory($category);
            if (!$data['smbpp_path']) return $link;
          }
          break;
          
        case 'extension/ocdevwizard/smart_blog_pro_plus/author':
        // end: OCdevWizard SMBPP
        ...
        // start: OCdevWizard SMBPP
        case 'smbpp_post_id':
        case 'smbpp_category_id':
        case 'smbpp_author_id':
        // end: OCdevWizard SMBPP
...
case 'album_id': 
        ...
        case 'smbpp_path':
          ...
          foreach ($categories as $category) {
            $queries[] = 'smbpp_category_id=' . $category;
          }
          unset($data[$key]);
          break;
        // end: OCdevWizard SMBPP
        ...
        // start: OCdevWizard SMBPP
        private function getSMBPPPathByPost($post_id) {
          $post_id = (int)$post_id;
          if ($post_id < 1) return false;
      
          static $path = null;
          if (!isset($path)) {
            $path = $this->cache->get('smbpp_post.seopath');
            if (!isset($path)) $path = array();
          }
      
          if (!isset($path[$post_id])) {
            $query = $this->db->query("SELECT smbpp_main_category_id FROM " . DB_PREFIX . "smbpp_post WHERE smbpp_post_id = '" . $post_id . "' ORDER BY smbpp_main_category_id DESC LIMIT 1");
      
            $path[$post_id] = $this->getSMBPPPathByCategory($query->num_rows ? (int)$query->row['smbpp_main_category_id'] : 0);
      
            $this->cache->set('smbpp_post.seopath', $path);
          }
      
          return $path[$post_id];
        }
      
        private function getSMBPPPathByCategory($category_id) {
          $category_id = (int)$category_id;
          if ($category_id < 1) return false;
      
          static $path = null;
          if (!isset($path)) {
            $path = $this->cache->get('smbpp_category.seopath');
            if (!isset($path)) $path = array();
          }
          ...
          if (!isset($path[$category_id])) {
            ...
            $sql = "SELECT CONCAT_WS('_'";
            for ($i = $max_level-1; $i >= 0; --$i) {
              $sql .= ",t$i.smbpp_category_id";
            }
            $sql .= ") AS smbpp_path FROM " . DB_PREFIX . "smbpp_category t0";
            for ($i = 1; $i < $max_level; ++$i) {
              $sql .= " LEFT JOIN " . DB_PREFIX . "smbpp_category t$i ON (t$i.smbpp_category_id = t" . ($i-1) . ".parent_id)";
            }
            $sql .= " WHERE t0.smbpp_category_id = '" . $category_id . "'";
      
            $query = $this->db->query($sql);
      
            $path[$category_id] = $query->num_rows ? $query->row['smbpp_path'] : false;
      
            $this->cache->set('smbpp_category.seopath', $path);
          }
