	...
	public function getProductName($get_data) {
		...
		foreach($all_languages as $lang) {
			if(version_compare(VERSION, '2.2.0.0', "<")) {
				$images[$lang['language_id']] = 'view/image/flags/'.$lang['image'];
	...
	public function getProductOrders($product_id) {
		...
		if ($query->num_rows) {
			foreach ($query->rows as $result) {
				$order_query = $this->db->query("SELECT o.order_id, o.firstname, o.lastname, o.total, o.date_added, os.name AS order_status FROM " . DB_PREFIX . "order o LEFT JOIN " . DB_PREFIX . "order_status os ON o.order_status_id = os.order_status_id WHERE o.order_id='" . (int)$result['order_id'] . "'");
