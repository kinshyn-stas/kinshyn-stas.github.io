    	...
    	<div class="panel panel-default">
      		<div class="panel-heading">
        		<div class="input-group">
					<span class="input-group-addon" style="min-width: 150px"><i class="fa fa-pencil"></i> <?php echo $text_editfor; ?></span>
				...
				<div class="tab-content">
					<div class="tab-pane active in" id="tab-items">
				  		<div class="table-responsive">
							<table id="item" class="table table-striped table-bordered table-hover" style="display: none">
							<thead>
								<tr>
									...
									<tr>
										<td class="text-left" style="background: <?php echo $libg; ?>;">
											<select style="border: 4px solid <?php if ($item['type'] == 'cat') { ?>#C85555<?php } elseif ($item['type'] == 'infol') { ?>#7593D9<?php } elseif ($item['type'] == 'infod') { ?>#D49ECA<?php } elseif ($item['type'] == 'custom') { ?>#F4D98E<?php } elseif ($item['type'] == 'mand') { ?>#A0C183<?php } elseif ($item['type'] == 'products') { ?>#8F6E96<?php } elseif ($item['type'] == 'catprods') { ?>#C5B15F<?php } elseif ($item['type'] == 'login') { ?>#9DC58D<?php } else { ?>#80ACAB<?php } ?>;" class="item-type" id="item-type<?php echo $item_row; ?>" name="supermenu_item[<?php echo $item_row; ?>][type]">
												...
												<?php } ?>
												<?php if ($item['type'] == 'catcatprods') { ?>
													<option class="catcatprods" id="catcatprods<?php echo $item_row; ?>" value="catcatprods" selected="selected"><?php echo $type_catcatprods; ?></option>
												<?php } else { ?>
													<option class="catcatprods" id="catcatprods<?php echo $item_row; ?>" value="catcatprods"><?php echo $type_catcatprods; ?></option>
												<?php } ?>
												...
												<div class="advh">
													...
													<table class="smenua">
														...
														<tr>
															...
															<td colspan="3"><input size="48" type="text" name="supermenu_item[<?php echo $item_row; ?>][fbrands]" value="<?php echo isset($item['fbrands']) ? $item['fbrands'] : ''; ?>" /></td>
														</tr>
													</table>
												</div>
											</div>
					...
					<div class="row">
						<div class="col-sm-5 col-xs-12" >
							...
							<div class="settings-row">
								...
								<input type="text" name="supermenu_image_width" value="<?php echo $supermenu_image_width; ?>" size="3" />px 
								<input type="text" name="supermenu_image_height" value="<?php echo $supermenu_image_height; ?>" size="3" />px
							</div>
							<div class="settings-row">
...
function additem() {	
	...
	html += '      <option class="catprods" id="catcatprods' + item_row + '" value="catcatprods"><?php echo $type_catcatprods; ?></option>';
	...
	$("#item-row" + item_row + " .item-type").change();
	item_row++;
	startColMan();
}

//activate first lang tab
$('#language a:first').tab('show');

//trigger change on item type
...
$(document).on('change','.item-row .item-type',function(){
  ...
  parent.find('.typediv').hide();
  if ($(this).find('.ocat').is(":selected")) {
     parent.find('.cat-type').fadeIn('fast');
     parent.find('.viewsel').fadeIn('fast');
  } else if ($(this).find('.oinfol').is(":selected")) {
     parent.find('.info-type').fadeIn('fast');
  } else if ($(this).find('.ocustom').is(":selected")) {
     parent.find('.custom-type').fadeIn('fast');
     ...
     parent.find('.oproducts-type').fadeIn('fast');
     parent.find('.viewsel').fadeIn('fast');
     parent.find('.productlimit-type').fadeIn('fast');
  } else if ($(this).find('.catprods').is(":selected") || $(this).find('.catcatprods').is(":selected")) {
     parent.find('.cat-type').fadeIn('fast');
     parent.find('.viewsel').fadeIn('fast');
     parent.find('.productlimit-type').fadeIn('fast');
  }
