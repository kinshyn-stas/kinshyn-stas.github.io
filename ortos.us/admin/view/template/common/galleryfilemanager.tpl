    ...
    <div class="modal-body">
      <div class="row">
        <div class="col-sm-5">
          ...
          <button type="button" data-toggle="tooltip" title="Добавить изображения" id="btn_mutil_img_add" class="btn btn-success"><i class="fa fa-plus"></i></button>
        </div>
        <div class="col-sm-7">
	  ...
	  <div class="row" style="padding-top: 10px; padding-left: 15px;" id="rad_mutil_img_select_div">
      	<input type="checkbox" onclick="$('input[name*=\'path\']').prop('checked', this.checked);"> Выбрать все </input>
	  </div>
      <hr />
...
function addImageHtml(obj) {
	...
	html += '  <td class="text-left"><a href="" id="thumb-image' + image_row + '"data-toggle="image" class="img-thumbnail"><img src="' + $(obj).find('img').attr('src') + '" alt="" title="" data-placeholder="<?php echo $placeholder; ?>" /><input type="hidden" name="album_image[' + image_row + '][image]" value="' + $(obj).parent().find('input').attr('value') + '" id="input-image' + image_row + '" /></td>';
...
function addUnitImageHtml(imagePath, imageUrl) {
	html  = '<tr id="image-row' + image_row + '">';
	html += '  <td class="text-left"><a href="" id="thumb-image' + image_row + '"data-toggle="image" class="img-thumbnail"><img src="' + imageUrl + '" alt="" title="" data-placeholder="<?php echo $placeholder; ?>" /><input type="hidden" name="album_image[' + image_row + '][image]" value="' + imagePath + '" id="input-image' + image_row + '" /></td>';
	 html += '  <td class="left"><textarea class="form-control" name="album_image[' + image_row + '][name]" cols="40" rows="3" /></textarea></td>';
	html += '  <td class="text-right"><input type="text" name="album_image[' + image_row + '][sort_order]" value="" placeholder="<?php echo $entry_sort_order; ?>" class="form-control" /></td>';
	html += '  <td class="text-left"><button type="button" onclick="$(\'#image-row' + image_row  + '\').remove();" data-toggle="tooltip" title="<?php echo $button_remove; ?>" class="btn btn-danger"><i class="fa fa-minus-circle"></i></button></td>';
	html += '</tr>';
...
$('#button-search').on('click', function() {
	var url = 'index.php?route=common/galleryfilemanager&token=<?php echo $token; ?>&directory=<?php echo $directory; ?>' + '&image_row=' + image_row;
...
$('#button-upload-multi').on('click', function() {
	...
	$('body').prepend('<input id="fileupload" type="file" name="files[]" style="display: none;" data-url="index.php?route=common/galleryfilemanager/multiupload&token=<?php echo $token; ?>&directory=' + upload_dir + '" multiple>');
...
$('#button-folder').on('shown.bs.popover', function() {
	$('#button-create').on('click', function() {
		$.ajax({
			url: 'index.php?route=common/galleryfilemanager/folder&token=<?php echo $token; ?>&directory=<?php echo $directory; ?>',
			type: 'post',		
			dataType: 'json',
...
$('#modal-image #button-delete').on('click', function(e) {
	if (confirm('<?php echo $text_confirm; ?>')) {
		$.ajax({
			url: 'index.php?route=common/galleryfilemanager/delete&token=<?php echo $token; ?>',
			type: 'post',		
			dataType: 'json',
