...
function uploadFiles(e) {
	...
	if (status) {
		$.ajax({
			url: 'index.php?route=' + productManagerPath + '/bulkupload&token=' + token,
			type: 'POST',
			data: data,
			cache: false,
			dataType: 'json',
...
$(document).ready(function(){
	 $.ajax({
		url: "index.php?route=" + productManagerPath + "/getList&token=" + token,
		type: 'get',
		dataType: 'html',
...
function getURL(curr_page) {
	...
	var url = 'index.php?route=' + productManagerPath + '/getList&token=' + token + '&page=' + curr_page + '&sort=' + sort + '&order=' + order;
...
$(document).ready(function(){
	$('#button-filter').on('click', function() {
		var url = 'index.php?route=' + productManagerPath + '/getList&token=' + token;
...
$(document).ready(function(){
	$('input[name=\'filter_name\']').autocomplete({
		'source': function(request, response) {
			$.ajax({
				url: 'index.php?route=' + productManagerPath + '/autocomplete&token=' + token + '&filter_name=' +  encodeURIComponent(request),
				dataType: 'json',
...
$(document).ready(function(){
	$('input[name=\'filter_model\']').autocomplete({
		'source': function(request, response) {
			$.ajax({
				url: 'index.php?route=' + productManagerPath + '/autocomplete&token=' + token + '&filter_model=' +  encodeURIComponent(request),
				dataType: 'json',
				success: function(json) {
					response($.map(json, function(item) {
						return {
...
function doSaveBulk(pid, pdata, pvalue, plang, ptype) {
	plang = plang || 0;
	$.ajax({
		url: "index.php?route=" + productManagerPath + "/updateproductbulk&token=" + token,
		type: "POST",
		dataType: "json",
		data: {
			pid: pid,
				...
				for (i = 0; i<resp['products'].length; i++) {
					...
					// stock_status & manufacturer & weight_class & tax_class
						...
						if (resp['products'][i]['categories'].length > 0) {
							var categories = '';
							for (z = 0; z<resp['products'][i]['categories'].length;z++) {
...
function doSave(pid, pdata, pvalue, plang) {		
	...
	$.ajax({
		url: "index.php?route=" + productManagerPath + "/updateproduct&token=" + token,
		type: "POST",
		dataType: "json",
		data: {
			pid: pid,
...
function editable() {
	...
	$('#form-product').on('click', '.bulk-edit', function(e) {
		...
		if (pid.length <= 1) {
			...
			if (pdata == 'name') {
				...
				var element = $(this).parents('td');
				$.ajax({
					url: "index.php?route=" + productManagerPath + "/getproductcategories&token=" + token,
					type: "POST",
					dataType: "json",
					data: {
						pid: pid,
					...
					success: function (resp) {
						element.append('<div class="after-edit-bulk" data-key="' + pdata + '" data-id="' + pid + '"><select class="form-control" style="margin-top: 5px; margin-bottom: 5px;" name="bulk_cat_type"><option value="addCats">Add selected categories as new ones</option><option value="replaceCats">Replace categories with the selected ones</option><option value="deleteCats">Delete selected categories</option></select><input type="text" name="category-bulk" value="" id="input-category" class="form-control" autocomplete="off"><div id="bulk-category" class="well well-sm after-edit well-category" style="height: 85px; text-align: left;     font-weight: normal; overflow: auto;">');
				...
				$('#module-edit-custom-modal #module-edit-custom-iframe').attr('src', 'index.php?route=' + productManagerPath + '/showSpecialsBulkForm&product_id=0&token=' + token);
				$('#module-edit-custom-modal').show();
				
				$('#module-edit-custom-modal #module-edit-custom-iframe').on('load', function(e) {
				...
				$('#module-edit-custom-modal #module-edit-custom-iframe').attr('src', 'index.php?route=' + productManagerPath + '/showAttributesBulkForm&product_id=0&token=' + token);
				$('#module-edit-custom-modal').show();

			 	$('#module-edit-custom-modal #module-edit-custom-iframe').load(function(e) {			 	
			 		var iframe = $('#module-edit-custom-modal #module-edit-custom-iframe');
			 		$('<input />').attr('type', 'hidden') 
			          .attr('name', "product_ids")
			          .attr('value', pid.toString())
				...
				$('#module-edit-custom-modal #module-edit-custom-iframe').attr('src', 'index.php?route=' + productManagerPath + '/showOptionsBulkForm&product_id=0&token=' + token);
				$('#module-edit-custom-modal').show();
	...
	$('#form-product').on('dblclick', '.edit-td', function(e){
		...
		// status & shipping and subtract
			...
			var element = $(this);
			$.ajax({
				url: "index.php?route=" + productManagerPath + "/getproductname&token=" + token,
				type: "POST",
				dataType: "json",
				data: {
					pid: pid,
	...
	$('#form-product').on('blur', '.after-edit-bulk', function(e){
		...
		if (!$(e.relatedTarget).is('[data-key="' + pdata + '"]') || $(e.relatedTarget).length == 0) {
			if (pdata == 'category') {
				if ($(temp_target).parents('.after-edit-bulk[data-key="category"]').length > 0 || $(temp_target).is('.after-edit-bulk[data-key="category"]')) {
					return false;
				}
			}
			
			var t = confirm(confirm_bulk);
	...
	$('#form-product').on('click', 'a[data-toggle=\'image-manager\']', function(e) {
		...
		$('.button-image-click', parent).on('click', function() {
			...
			$.ajax({
				url: 'index.php?route=common/filemanager&token=' + getURLVar('token') + '&target=' + $(element).parent().find('input').attr('id') + '&thumb=' + $(element).attr('id') + '&productmanager=1',
				dataType: 'html',
				beforeSend: function() {
	...
	$('#form-product').on('click', '.module-edit', function(e) {
		...
		$('#module-edit-modal #module-edit-iframe').attr('src', 'index.php?route=catalog/product/edit&product_id=' + product_id + '&token=' + token);
	});
	...
	$('#form-product').on('click', '.view-orders', function(e) {
		e.preventDefault();
		var product_id = $(this).parents('.edit-tr').attr('data-id');
		$('#module-edit-custom-modal #module-edit-custom-iframe').attr('src', 'index.php?route=' + productManagerPath + '/showOrdersPopup&product_id=' + product_id + '&token=' + token);
	...
	$('#module-edit-custom-modal #module-edit-custom-iframe, #module-edit-custom-bulk-modal #module-edit-custom-bulk-iframe').load(function(event) {
		...
		if 	(current_url.indexOf(productManagerPath) > -1 && 
			...
			if(product_id == '0') {
				iframe.contents().find('form').attr('action',"index.php?route=" + productManagerPath + "/updateproductbulk&token=" + token);
			}					
	...
	$('#form-product').on('click', '.popup-specials', function(e) {
		e.preventDefault();
		var product_id = $(this).parents('.edit-tr').attr('data-id');
		$('#module-edit-custom-modal #module-edit-custom-iframe').attr('src', 'index.php?route=' + productManagerPath + '/showSpecialsForm&product_id=' + product_id + '&token=' + token);
	...
	$('#form-product').on('click', '.popup-attributes', function(e) {
		e.preventDefault();
		var product_id = $(this).parents('.edit-tr').attr('data-id');
		$('#module-edit-custom-modal #module-edit-custom-iframe').attr('src', 'index.php?route=' + productManagerPath + '/showAttributesForm&product_id=' + product_id + '&token=' + token);
	...
	$('#form-product').on('click', '.popup-options', function(e) {
		e.preventDefault();
		var product_id = $(this).parents('.edit-tr').attr('data-id');
		$('#module-edit-custom-modal #module-edit-custom-iframe').attr('src', 'index.php?route=' + productManagerPath + '/showOptionsForm&product_id=' + product_id + '&token=' + token);

		$('#module-edit-custom-modal #module-edit-custom-iframe').on("load", function(evev) {
		    evev.preventDefault();
			evev.stopPropagation();
			evev.stopImmediatePropagation();	
		});
