<?php
class ControllerCommonGalleryFileManager extends Controller {
	public function index() {
		$this->load->language('common/galleryfilemanager');
		...
		foreach ($images as $image) {
			...
			if (is_dir($image)) {
				...
				$data['images'][] = array(
					...
					'href'  => $this->url->link('common/galleryfilemanager', 'token=' . $this->session->data['token'] . '&directory=' . urlencode(utf8_substr($image, utf8_strlen(DIR_IMAGE . 'catalog/'))) . $url, true)
				);
		...
		$data['parent'] = $this->url->link('common/galleryfilemanager', 'token=' . $this->session->data['token'] . $url, true);
		...
		$data['refresh'] = $this->url->link('common/galleryfilemanager', 'token=' . $this->session->data['token'] . $url, true);
		...
		$pagination->limit = 16;
		$pagination->url = $this->url->link('common/galleryfilemanager', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);

		$data['pagination'] = $pagination->render();

		$this->response->setOutput($this->load->view('common/galleryfilemanager', $data));
	}
	...
	public function upload() {
		...
		if (!$this->user->hasPermission('modify', 'common/galleryfilemanager')) {
