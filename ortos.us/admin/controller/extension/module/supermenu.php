...
class ControllerExtensionModulesupermenu extends Controller {
	...
	public function index() {   
		...
		$this->language->load('extension/module/supermenu');
		...
		if (isset($this->request->get['deleteOldVersion']) && $this->validate()) {
			...
			$this->response->redirect($this->url->link('extension/module/supermenu', 'token=' . $this->session->data['token'], 'SSL'));
		} elseif (isset($this->request->get['deleteOldVersion'])) {
			$this->response->redirect($this->url->link('extension/module/supermenu', 'nopermissiontodelete=1&token=' . $this->session->data['token'], 'SSL'));
		}
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('extension/module/supermenu', 'token=' . $this->session->data['token'] . $redirect, 'SSL'));
		}
		...
		if (is_file(DIR_APPLICATION . 'controller/module/supermenu.php')) {
			...
			$data['error_warning'] .= '<a href="' . $this->url->link('extension/module/supermenu', 'deleteOldVersion=1&token=' . $this->session->data['token'], 'SSL') . '" class="btn btn-xs btn-danger">' . $this->language->get('delete_old_version') . '</a>';
		}
		if (!$this->validate()) {
		...
		if (isset($this->session->data['success'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/module/supermenu', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		if ($data['store']) {
		  $data['action'] = $this->url->link('extension/module/supermenu', 'token=' . $this->session->data['token'] . '&store=' . $data['store'], 'SSL');
		} else {
		  $data['action'] = $this->url->link('extension/module/supermenu', 'token=' . $this->session->data['token'], 'SSL');
		}

		$data['module_url'] = $this->url->link('extension/module/supermenu', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['supermenu_more'])) {
			...
			$data['supermenu_more'] = $this->getSetting('supermenu_more');
		} else {
		...
		if (isset($this->request->post['supermenu_more2'])) {
			...
			$data['supermenu_more2'] = $this->getSetting('supermenu_more2');
		} else {
		...
		$this->response->setOutput($this->load->view('extension/module/supermenu.tpl', $data));
	}
