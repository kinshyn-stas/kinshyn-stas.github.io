<?php
class ControllerExtensionShippingDostavkaPlus extends Controller
{
    protected $error = array();
    protected $type = 'shipping';
    protected $newpath = 'extension/shipping';
    protected $name = 'dostavkaplus';
    protected $arr_bibb = array(0 => 'zone', 1 => 'quantity', 2 => 'total', 3 => 'weight');

    function index()
    {
        $data                        = $this->load->language($this->newpath . '/' . $this->name);
        ...
        foreach ($order_totals as $ot) {
            ...
            if ($ot['code'] != $this->type) {
                if ($this->config->get($ot['code'] . '_status')) {
                    $this->load->language('extension/total/' . $ot['code']);
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validate())) {
            ...
            $this->response->redirect($this->url->link('extension/extension', 'token=' . $this->session->data['token'].'&type='.$this->type, true));
        }
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link($this->newpath . '/' . $this->name, 'token=' . $this->session->data['token'], true)
        );
        $data['action']        = $this->url->link($this->newpath . '/' . $this->name, 'token=' . $this->session->data['token'], true);
        ...
        $this->response->setOutput($this->load->view($this->newpath . '/' . $this->name, $data));
    }

    function validate()
    {
        if (!$this->user->hasPermission('modify', $this->newpath . '/' . $this->name)) {
    ...
    function license()
    {
        $data         = $this->load->language($this->newpath . '/' . $this->name . '_license');
        $data['name'] = $this->name;
        $this->load->model('localisation/language');
        $data['languages'] = $this->model_localisation_language->getLanguages();
        $this->document->setTitle($this->language->get('heading_title'));
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateKey())) {
            $this->model_setting_setting->editSetting($this->name, $this->request->post);
            $this->response->redirect($this->url->link($this->newpath . '/' . $this->name, 'token=' . $this->session->data['token'], true));
        }
        ...
        $data['breadcrumbs'][] = array(
            'text' => $this->language->get('heading_title'),
            'href' => $this->url->link($this->newpath . '/' . $this->name . '/license', 'token=' . $this->session->data['token'], true)
        );
        $data['action']        = $this->url->link($this->newpath . '/' . $this->name . '/license', 'token=' . $this->session->data['token'], true);
        $data['cancel']        = $this->url->link('extension/extension', 'token=' . $this->session->data['token'], true);
        ...
        $data['column_left'] = $this->load->controller('common/column_left');
        $data['footer']      = $this->load->controller('common/footer');
        $this->response->setOutput($this->load->view($this->newpath . '/' . $this->name . '_license', $data));
    }
