		...
		if (isset($this->request->get['search'])) {
			...
			foreach ($results as $result) {
				...
				$data['products'][] = array(
					...
					'thumb'       => $image,
					'name'        => $result['name'],
					'description' => $this->model_catalog_isearch->isearch_substr(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8')), 0, $this->config->get($this->config->get('config_theme') . '_product_description_length')) . '...',
					'price'       => $price,
					'model'				=> $result['model'],
					...
					'href'        => $this->url->link('product/product', $url . '&product_id=' . $result['product_id'], true)
				);
			}
			...
			$data['limits'][] = array(
				'text'  => $this->config->get($this->config->get('config_theme') . '_product_limit'),
				'value' => $this->config->get($this->config->get('config_theme') . '_product_limit'),
				'href'  => $this->url->link('product/isearch', $url . '&limit=' . $this->config->get($this->config->get('config_theme') . '_product_limit'))
			);
