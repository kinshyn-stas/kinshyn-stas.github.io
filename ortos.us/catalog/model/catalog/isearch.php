        ...
        if (!$products) {
            ...
            if($this->isearch_strlen($keywords) >= 2) {
                ...
                if ($searchIn['optionvalue'] === true) { 
                    ...
                    //if ($searchIn['optionname'] !== true) {
						$extra_sql .= 'LEFT JOIN ' . DB_PREFIX . 'product_option_value AS poptv ON poptv.product_id = pd.product_id ';
                        
                   // }
						$extra_sql .= 'LEFT JOIN ' . DB_PREFIX . 'option_value_description AS ovde ON (ovde.option_value_id = poptv.option_value_id AND pd.language_id = ovde.language_id)';
                }
