<?php
    class ModelProgromanFias extends Model {
        ...
        public function findFiasByName($term) {
            ...
            $sql = "SELECT CONCAT_WS(', ', CONCAT(f1.shortname, ' ', f1.offname), CONCAT(f2.offname, ' ', f2.shortname), CONCAT(f3.offname, ' ', f3.shortname)) `label`, 
                      CONCAT(f1.shortname, ' ', f1.offname) `name`, f1.fias_id `value`
                    FROM fias f1
                        LEFT JOIN fias f2 ON f2.fias_id = f1.parent_id
                        LEFT JOIN fias f3 ON f3.fias_id = f2.parent_id
                        WHERE (%WHERE%)
                      AND (f1.level = " . self::LEVEL_CITY . " OR f1.level = " . self::LEVEL_TOWN
                        . " OR (f1.level = " . self::LEVEL_REGION. " AND f1.shortname = 'г.'))
            ...
            if (!empty($filter['city_name'])) {
                return $this->db->query("
                    ...
                    WHERE f1.offname = '" . $this->db->escape($filter['city_name']) . "'
                        ...
                        AND (f1.level = " . self::LEVEL_CITY . " OR (f1.level = " . self::LEVEL_REGION . " AND f1.shortname = 'г.'))")->row;
            }
            else {
                return $this->db->query("
