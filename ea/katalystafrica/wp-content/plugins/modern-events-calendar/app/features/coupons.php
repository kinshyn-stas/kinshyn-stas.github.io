...
class MEC_feature_coupons extends MEC_base
{
    ...
    ?>
        ...
        <tr class="form-field">
            ...
            <td>
                <input type="text" name="expiration_date" id="mec_expiration_date" value="<?php echo $expiration_date; ?>" class="mec_date_picker" />
                <p class="description"><?php _e('Leave it empty for no expiration!', 'mec'); ?></p>
            </td>
        </tr>
        <tr class="form-field">
            <th scope="row">
            ...
            <td>
                <select name="target_event" id="mec_target_event" style="width: 95%;">
                    <option value=""><?php _e('All Events', 'mec'); ?></option>
                    <?php foreach($events as $event): ?>
                        <option value="<?php echo $event->ID; ?>" <?php echo ($target_event == $event->ID ? 'selected="selected"' : ''); ?>><?php echo $event->post_title; ?></option>
                    <?php endforeach; ?>
                </select>
            </td>
        </tr>
    ...
    public function save_metadata($term_id)
    {
        ...
        $expiration_date = (isset($_POST['expiration_date']) and trim($_POST['expiration_date'])) ? $_POST['expiration_date'] : '';
        update_term_meta($term_id, 'expiration_date', $expiration_date);

        $target_event = (isset($_POST['target_event']) and trim($_POST['target_event'])) ? $_POST['target_event'] : '';
    ...
    public function apply_coupon()
    {
        ...
        if(!wp_verify_nonce($_POST['_wpnonce'], 'mec_apply_coupon_'.$transaction_id)) $this->main->response(array('success'=>0, 'code'=>'NONCE_IS_INVALID'));
        ...
        $validity = $this->book->coupon_check_validity($coupon, $event_id);
        ...
        elseif($validity == -1) $this->main->response(array('success'=>0, 'code'=>'COUPON_USAGE_REACHED', 'message'=>__('Discount coupon usage limit reached!', 'mec')));
        // Coupon is expired!
