	...
	public static function log($outfile, $err)
	{
		if (QR_LOG_DIR !== false) {
			if ($err != '') {
				...
				if ($outfile !== false) {
					MEC_file::write(QR_LOG_DIR.basename($outfile).'-errors.txt', $err);
				} else {
                    MEC_file::write(QR_LOG_DIR.'errors.txt', $err);
				}
			}    
		}
	}
		...
		if(!isset(self::$frames[$version])) {
			...
			if (QR_CACHEABLE) {
				if (file_exists($fileName)) {
					self::$frames[$version] = self::unserial(MEC_file::read($fileName));
				} else {
					self::$frames[$version] = self::createFrame($version);
                    MEC_file::write($fileName, self::serial(self::$frames[$version]));
				}
			} else {
				self::$frames[$version] = self::createFrame($version);
			}
...
class QRmask {
	...
	public function makeMaskNo($maskNo, $width, $s, &$d, $maskGenOnly = false) 
	{
		...
		if (QR_CACHEABLE) {
			if (file_exists($fileName)) {
				$bitMask = self::unserial(MEC_file::read($fileName));
			} else {
				$bitMask = $this->generateMaskNo($maskNo, $width, $s, $d);
				if (!file_exists(QR_CACHE_DIR.'mask_'.$maskNo))
					mkdir(QR_CACHE_DIR.'mask_'.$maskNo);
                MEC_file::write($fileName, self::serial($bitMask));
			}
		} else {
			$bitMask = $this->generateMaskNo($maskNo, $width, $s, $d);
		}
