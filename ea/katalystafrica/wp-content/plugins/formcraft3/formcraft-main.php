<?php
  ...
  */
  ...
  $fc_meta['user_can'] = strpos(get_site_url(), 'formcraft-wp.com/demo') !== false ? 'read' : 'activate_plugins';
  $fc_meta['preview_mode'] = strpos(get_site_url(), 'formcraft-wp.com/demo') !== false ? true : false;
  ...
  */
  ...
  function formcraft3_redirect_to_form_page()
  {
    ...
    function formcraft3_shortcode( $atts, $content = '' ) {
      ...
      wp_enqueue_script('fc-modal-js', plugins_url( 'assets/js/src/fc_modal.js', __FILE__ ), array('jquery'));
      wp_enqueue_script('fc-tooltip-js', plugins_url( 'assets/js/vendor/tooltip.min.js', __FILE__ ), array('jquery', 'fc-modal-js'));
      wp_enqueue_script('fc-autocomplete', plugins_url( 'lib/awesomplete.min.js', __FILE__ ));
      wp_enqueue_script('fc-form-js', plugins_url( 'dist/form.min.js', __FILE__ ), $dependencies, $fc_meta['version']);
  ...
  */
  ...
  function formcraft_new_form() {
    ...
    switch ($_POST['type']) {
      ...
      $formData['addons'] = json_decode(stripcslashes($existing_form['addons']));
  ...
  */
  ...
  function formcraft3_load_form_data() {
    ...
    if ($_GET['type']=='builder') {
      ...
      echo json_encode(array('meta_builder'=>$meta,'builder'=>$builder,'addons'=>$addons,'name'=>$name,'old_url'=>$old_url,'new_url'=>site_url()));
    }
    die();
  }
  ...
  add_action( 'wp_ajax_formcraft_toggle_analytics', 'formcraft_toggle_analytics' );
  function formcraft_toggle_analytics() {
    global $fc_meta, $wpdb;
    ...
    if (isset($meta['config'])) {
      if (isset($meta['config']['notifications']['recipients'])) {
        ...
        if (is_array($emails) && count($emails) > 0) {
          ...
          if ($failed>0) {
            $fc_final_response['debug']['failed'][] = __('Email Not Sent: ','formcraft').$failed_msg;
          }
          if ($sent>0) {
            $fc_final_response['debug']['success'][] = __($sent.' notification email(s) sent','formcraft');
          }
        }
      }
    }
  ...
  function formcraft3_admin_assets($hook) {
    ...
    if ( strpos($hook, 'formcraft-dashboard') !== FALSE && isset($_GET['id']) ) {
      ...
      if (in_array($page, $load_react)) {
        ...
        wp_enqueue_script('chart-js', plugins_url( 'assets/js/vendor/chart.min.js', __FILE__ ));
        // Load MomentJS
