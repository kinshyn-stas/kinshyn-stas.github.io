	...
	jQuery.ajax({
		...
		.done(function(response) {
			...
			for (let type in addons) {
				...
				if (type === 'free') {
					addonType = 'Free AddOns'
				}
				if (type === 'purchased') {
					addonType = 'Purchased AddOns'
				}
				if (type === 'other') {
					addonType = 'Paid AddOns'
				}
				...
				for (let addon in addons[type]) {
					...
					if (addons[type][addon].price === 0 || addons[type][addon].purchased === true) {
						button = `<button class="toggle-install formcraft-button small" data-plugin="${addons[type][addon].ID}"><span>Install Plugin</span><div class='formcraft-loader'></div></button><a class="read-more-addon" href="http://formcraft-wp.com/addons/?page_id=${addons[type][addon].ID}" target="_blank">read more</a>`
					} else {
		...
		.fail(function() {
			jQuery('.new-addons').html('')
			globalNotification('error', 'Please check your internet connection')
		})
}
...
function updateHelp(query, type, log) {
	...
	.done(function(response) {
		...
		let html = '<div id="help-top"><span class="formcraft-button small" id="help-back">← back</span><span class="formcraft-button small" id="help-home">Index</span><span class="formcraft-button small close" data-dismiss="fc_modal" aria-label="Close">close</span></div><div class="article-content">'
		if (type === 'categories') {
			let htmlListElement = ''
			if (query.indexOf('search=') !== -1) {
				...
				html = `${html}<h2>Help Topics</h2><div style="padding-bottom: 10%">`
			}
			let HTMLarray = []
		...
		jQuery.ajax({
			...
			.done(function(response) {
				if (response.failed) {
					...
					globalNotification('error', 'Unknown Error.')
					jQuery(this).removeClass('loading').removeAttr('disabled')
				}
			})
			.fail(function() {
	...
	jQuery.ajax({
		...
		.done(function(response) {
			if (response.failed) {
				...
				globalNotification('error', 'Failed Saving. Unknown Error.')
			}
		})
		.fail(function(response, status, code) {
			if (code === 'Forbidden') {
				globalNotification('error', 'Failed Saving. Please try disabing your firewall, or security plugin.')
			} else {
				globalNotification('error', 'Failed Saving')
			}
		})
		.always(function() {
	...
	$scope.testEmail = function() {
		...
		$http({
			...
			params: {
				...
				config: $scope.Builder.Config.notifications
			}
		})
			.success((response) => {
				if (response.failed) {
					$scope.TestEmailResult = `<div class="IsRed">${response.failed}</div>`
					$scope.TestEmailResultMore = `<br/><strong>Debug Info:</strong><br/>${response.debug}`
				} else {
