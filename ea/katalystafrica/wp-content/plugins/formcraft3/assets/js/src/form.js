...
class FormCraft {
	...
	constructor(form) {
		...
		let logicText = this.parentElement.find('.form-logic').text().replace(/“/g, '"').replace(/”/g, '"').replace(/″/g, '"').replace(/„/g, '"').replace(/″/g, '"')
	...
	calculateAndApplyMath(formula) {
		...
		if (formula.variables.length === 1 && formula.variables[0] === formula.string) {
			...
			if (jQuery(`#bind-math-${formula.identifier}`).prop('type') === 'hidden') {
				...
				helpers.spinTo(`#bind-math-${formula.identifier}`, mathResult, thousand, decimal)
			} else {
		...
		for (let x in conditions) {
			...
			switch (conditions[x][1]) {
				...
				if (conditionToCheck.toString().indexOf('-') === 4) {
					...
					//console.log(tempVal, value.toString())
